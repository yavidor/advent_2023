...
func main() {
	...
	var reGames = regexp.MustCompile(`(?m)Game (\d{1,3})`)
	var str = `Game 1: 4 green, 7 blue; 2 blue, 4 red; 5 blue, 2 green, 2 red; 1 green, 3 red, 9 blue; 3 green, 9 blue; 7 green, 2 blue, 2 red
...
Game 100: 2 blue, 12 green; 6 green, 1 red, 12 blue; 1 green, 5 blue, 1 red; 1 red, 12 green, 6 blue; 16 blue, 3 green`
	...
	for i, match := range reBalls.FindAllString(str, -1) {
		fmt.Println(match, "found at index", i)
	}
	for i, match := range reGames.FindAllString(str, -1) {
		fmt.Println(match, "found at index", i)
	}
	found := reGames.FindAllStringSubmatch(str, -1)
	for _, v := range found {
		fmt.Println(v[1])
	}
}
